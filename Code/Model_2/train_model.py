...
def map_join(vector):
	...
	for element in vector:
		if element < 0:
			string += chr(65-element-1)
		else:
			string += chr(65+discrete_levels-1+element-1)
...
def point_level_kernel_function(vec1, vec2, ic, dc, tc, sc):
	...
	fin_dist = np.exp([-dist_exp_coeff*dist**2])[0]
...
if __name__ == '__main__':
	...
	model = SVC(decision_function_shape='ovo', C=30, kernel=model2_kernel)
	scores = cross_val_score(model, x_set, y_set, cv=4)
	print "Cross Val Avg Accuracy: ", sum(scores) / len(scores)

	# Train & Save model
	model = SVC(decision_function_shape='ovo', C=30, kernel=model2_kernel, probability=True)
